...
Constraints: Python >= 3.11 on the machine where workers run. Version 1.8.1
...
python -m pip install \
  "orchestrator-engine @ git+https://github.com/Jafa7/OrchestratorEngine.git@v1.8.1"
```
...
The installed v1.8.1 package includes `orchestrator-engine conformance run`.
...
path for diagnosis. This is a required check for the v1.8.1 installation;
older pinned releases that lack the command need their own upgrade procedure.

The v1.8.1 CI runs native lifecycle, resource coordination, full conformance and
