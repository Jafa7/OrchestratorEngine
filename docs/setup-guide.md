...
Constraints: Python >= 3.11 on the machine where workers run. Version 1.9.2
...
The installed v1.9.2 package includes `orchestrator-engine conformance run`.
...
path for diagnosis. This is a required check for the v1.9.2 installation;
older pinned releases that lack the command need their own upgrade procedure.

The v1.9.2 CI runs native lifecycle, resource coordination, full conformance and
