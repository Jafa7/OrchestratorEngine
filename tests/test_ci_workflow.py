...
class CIWorkflowTests(unittest.TestCase):
    def test_ci_runs_bounded_soak_and_pinned_upgrade_matrix(self) -> None:
        ...
        for marker in (
            ...
            "tools/run_native_acceptance.py",
            "env -u PYTHONPATH python tools/run_native_acceptance.py",
            "--iterations 5 --timeout-seconds 15",
            "--expected-system Darwin",
            "--expected-system Windows",
            "actions/upload-artifact@v4",
            "native-acceptance-${{ matrix.os }}-py${{ matrix.python }}",
            "native-acceptance-windows-2022-py${{ matrix.python }}",
            'python: "3.12"',
            'python-version: ["3.11", "3.12", "3.13"]',
            "python -m pip wheel . --no-deps --wheel-dir native-dist",
